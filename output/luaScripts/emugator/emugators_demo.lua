...
loadUnloadDiskButton = 84
insertEjectDiskButton = 85
switchDiskButton = 86
pinMode(loadUnloadDiskButton,INPUT)
...
--		print(buttonPressed)
		...
		if ((digitalRead(loadUnloadDiskButton) == 1) and (buttonPressed == false)) then
			gui.opacity(1.0)
			emu.closeRom()
			buttonPressed = true;
		end
		if ((digitalRead(insertEjectDiskButton) == 1) and (buttonPressed == false)) then
			emu.insertOrEjectDisk()
			buttonPressed = true;
--			pinMode(insertEjectDiskButton,OUTPUT)
--			digitalWrite(insertEjectDiskButton,LOW)			
		end		
		if ((digitalRead(switchDiskButton) == 1) and (buttonPressed == false)) then
			emu.switchDisk()
			buttonPressed = true;
--			pinMode(switchDiskButton,OUTPUT)
--			digitalWrite(switchDiskButton,LOW)			
		end				
...
--			gui.drawimage(scanLine,0.3)
			gui.gdoverlay(0, 0, scanLine,1.5)
		end
